...
int decrypt(char* srcFileName, char* outFileName)
{
	...
	if (result == Z_OK) {
		...
		while (1){
			...
			strBuff[b] = CHAR_NULL;
			// wcout << L"strBuff: " << strBuff << endl;
			fwprintf(outFile, L"%u\t%u\t%u\t%u\t%u\t\"", strType, strID1, strID2, strID3, strID4);
fwrite(strBuff, sizeof(wchar_t), wcslen(strBuff), outFile);
fwprintf(outFile, L"\"\r\n");
...
{
	// Chương trình chính bắt đầu từ đây
	if (argc >= 3)
	{
